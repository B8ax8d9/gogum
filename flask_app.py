# GoGum - Flask Web Application for PythonAnywhere & Cloud WSGI
import os
import asyncio
from flask import Flask, request, redirect, Response

from config import get_enabled_sites
from translator import expand_search_names
...
<head>
    ...
    <title>GoGum — Web Search Engine</title>
...
def search():
    ...
    # 1. Resolve names
    expanded_names = expand_search_names([query])
    sites = get_enabled_sites()

    # 2. Execute search
    include_google = (mode == 'deep')
    results = run_search(sites, expanded_names, include_google=include_google)

    dork_results = []
    if mode == 'deep':
        try:
            from osint import dork_all_sites
            dork_results = asyncio.run(dork_all_sites(sites, expanded_names))
        except Exception:
            pass
    ...
    # 4. Inject Search Navigation Bar
    injection_html = f"""
    <!-- GoGum Injected Search Hub -->
...
@app.route('/health')
def health():
    return {"status": "ok", "service": "GoGum Search"}
